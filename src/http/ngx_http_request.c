...
ngx_http_connection_t *get_my_http_connection(ngx_connection_t *c)
{
    ngx_uint_t              i;
    struct sockaddr_in     *sin;
    ngx_http_port_t        *port;
...
ngx_connection_t *get_my_connection(ngx_cycle_t *cycle, ngx_pool_t *pool)
{
    ngx_str_t localaddr = ngx_string("0.0.0.0:443");
    socklen_t socklen;
    struct sockaddr_in *mysock; //*lsock;
    ngx_listening_t *ls;
    ngx_connection_t  *c;
    ngx_uint_t i;
    //ngx_event_t *ne;
    if(my_server_addr_port){
	localaddr.data = (u_char *)my_server_addr_port;
        localaddr.len = sizeof(my_server_addr_port);
    }

    c = ngx_pcalloc(pool, sizeof(ngx_connection_t));

    socklen = sizeof(struct sockaddr_in);
    //lsock = ngx_pcalloc(pool, socklen);
    //lsock->sin_family = AF_INET;
    //lsock->sin_port = htons(80);
    //lsock->sin_addr.s_addr = inet_addr("0.0.0.0");

    ls = cycle->listening.elts; 
    for (i = 0; i < cycle->listening.nelts; i++) {
        if(ngx_strcmp(&ls[i].addr_text.data, &localaddr)){
            c->listening = &ls[i];
            break;
        }

	printf("not match listen port\n");
        exit(1);
    }
    /*
    ls = ngx_pcalloc(pool, sizeof(ngx_listening_t));
    ls->sockaddr = (struct sockaddr *) lsock;
    ls->socklen = socklen;
    ls->addr_text = localaddr;
    ls->type = SOCK_STREAM;
    */

    mysock = ngx_pcalloc(pool, socklen);
    mysock->sin_family = AF_INET;
    mysock->sin_port = htons(800);
    mysock->sin_addr.s_addr = inet_addr("127.0.0.1");

    c->sockaddr = ngx_palloc(pool, socklen);
    ngx_memcpy(c->sockaddr, mysock, socklen);
    c->log = cycle->log;
    c->pool = pool;
    c->pool->log = cycle->log;
    c->socklen = socklen;
    ...
    //ne = ngx_palloc(pool, sizeof(ngx_event_t));
    //ne->timer_set = 0;
    //c->read = ne;
    //c->write = ne;
...
static void
my_http_process_request_headers(ngx_http_request_t *r)
{
    ngx_int_t                   rc;
    ngx_table_elt_t            *h;
    ngx_connection_t           *c;
...
static void
my_http_process_request_line(ngx_http_request_t *r)
{
    ngx_int_t            rc;
    ngx_str_t            host;
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            my_http_process_request_headers(r);
...
void ngx_get_upstream_request( ngx_cycle_t *cycle){
    ngx_buf_t *my_ngx_request;
    ngx_pool_t *pool;
    ngx_connection_t  *c;
    ngx_http_connection_t  *hc;
    ngx_http_request_t *r;
    ...
    my_ngx_request = ngx_create_temp_buf(pool, 1024);
    my_ngx_request->last = ngx_sprintf(my_ngx_request->last, "GET %s HTTP/1.1\r\nHost: %s\r\nTest: left\r\n\r\n", purl->path, purl->host);
    //my_ngx_request->last = ngx_sprintf(my_ngx_request->last, my_request);

    c = get_my_connection(cycle, pool);
    hc = get_my_http_connection(c);
    r = ngx_http_create_request(c);
    r->header_in = my_ngx_request;
    my_http_process_request_line(r);
}
