...
//left
    ngx_str_t   *host;
    ngx_uint_t  i,j;
    ngx_pool_t *pool = ngx_create_pool(102400, r->connection->log);
    ngx_http_upstream_server_t    *server;
    ngx_http_upstream_srv_conf_t   *uscf, **uscfp;
    ngx_http_upstream_conf_t     *upcf;
    ...
    b->last = ngx_sprintf(b->last,
        ...
        upcf->connect_timeout,
        upcf->read_timeout,
        upcf->send_timeout
        );

    b->last = ngx_sprintf(b->last, "\"upstream\":\"%V\", \"server\":", host);
    if(uscf->servers){
        ...
        for(j = 0; j < uscf->servers->nelts;j++){
            ...
            b->last = ngx_sprintf(b->last,"{\"addr\":\"%V\", "
                "\"weight\":\"%ui\", "
                "\"down\":\"%d\", "
                "\"backup\":\"%d\"}",
                &(server[j].addrs->name),
