...
static char *ngx_set_cpu_affinity(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static void ngx_get_upstream_process(ngx_cycle_t *cycle);
extern void ngx_get_upstream_request(ngx_cycle_t *cycle);
static parsed_url_t  *get_parse_url(const char *url);
    ...
    if (ngx_show_version) {
        ...
        if (ngx_show_help) {
            ngx_write_stderr(
                ...
                "  -s listen addr  : set local listen addr " NGX_LINEFEED
...
*/
    ngx_use_stderr = 1;
//left
        ngx_get_upstream_process(cycle);
//end left
...
ngx_get_options(int argc, char *const *argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (*p++ != '-') {
           // ngx_log_stderr(0, "invalid option: \"%s\", please use -h for help!", argv[i]);
    		purl = get_parse_url((char *)(--p));
    		if(NULL == purl){
	    		printf("request error, please use -h for help!\n");
	    		exit(1);
    		}
        ...
        while (*p) {
            ...
            case 's':
                if (*p) {
                    my_server_addr_port = (char *) p;

                } else if (argv[++i]&&i<(argc-1)) {
                    my_server_addr_port = argv[i];

                } else {
                    ngx_log_stderr(0, "option \"-s\" listen addr");
                    return NGX_ERROR;
                }
            ...
            default:
                ...
                ngx_log_stderr(0, "invalid option: \"%c\", please use -h for help!", *(p - 1));
                return NGX_ERROR;
            }
        }
...
static parsed_url_t  *get_parse_url(const char *url)
{
    ...
    purl = malloc(sizeof(parsed_url_t));
    if ( NULL == purl ) {
        return NULL;
    }
    purl->scheme = NULL;
...
void
ngx_get_upstream_process(ngx_cycle_t *cycle)
{
    ngx_uint_t  i;
    ...
    ngx_str_t epoll_name = ngx_string("ngx_epoll_module");
    for (i = 0; ngx_modules[i]; i++) {
	if(ngx_strcmp(ngx_module_names[i], epoll_name.data)){
		continue;
	}
    ...
    ngx_get_upstream_request(cycle);
    exit(0);
}
